...
describe('GameplayCombatFlowService', () => {
    ...
    beforeEach(() => {
        ...
        service = new GameplayCombatFlowService(
            ...
            { isTurnInProgress: sinon.stub().returns(false) } as unknown as VirtualPlayerTurnFinalizerService,
        );
    });
    ...
    it('skips delayed combat resolution when the combat has already been cleared', async () => {
        ...
        await service.combatManager('game-1', 'Alice', 'Bob', null, { namespace, emitGameLog: sinon.stub() });
